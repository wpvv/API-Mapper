...
def string_preprocessing(value: str) -> str:
    ...
    return_value_list = [
        lemmatizer(word)
        for word in value
        if word not in stop_words + additional_stopwords
    ]
...
def calculate_similarity_scores(
    ...
    for similarity_type in ["path", "description"]:
        messages = [
            df_row["application1_" + similarity_type],
...
def calculate_json_glue_scores(df_row: dict) -> dict:
    ...
    """
    jsonglue_scores = JsonGlue.calculate_json_glue_score(
        df_row["application1_schema"], df_row["application2_schema"]
    )
...
def create_data_set(application_ids=None, training=False):
    ...
    if training:
        ...
        applications = [
            ApplicationConfig.get_application_config(application_id, internal=True)
        ...
        for path in specs["paths"]:
            for operation in specs["paths"][path]:
                ...
                api_data = {
                    ...
                    "operation": operation,
                    "original_description": get_description_and_summary(
                        temp_spec, preprocessing=False
                    ),
                ...
                if not training and endpoint_id is not None:
                    ...
                    api_data["server_override"] = (
                        ""
                        if "servers" not in temp_spec
                        else temp_spec["servers"][0]["url"]
                    )
                    endpoint_id += 1
    ...
    for element in data_combinations:
        ...
        for index, tuple_item in enumerate(element):
            row.update(
                {
                    "application" + str(index + 1) + "_" + str(key): val
    ...
    data_set = data_set.apply(
        lambda df_row: calculate_similarity_scores(model, df_row, training=training),
        axis=1,
    )
...
def read_training_data_set(
    ...
    print(
        "x_train shape:",
        train_data.drop("label", axis=1).shape,
        "| X_test shape:",
        test_data.drop("label", axis=1).shape,
    )
    print(
        "Label train mean:",
        numpy.mean(train_data["label"]).round(2),
        "| label test mean:",
...
def create_feature_graphs(data_set: pd.DataFrame, train_data: pd.DataFrame) -> None:
    ...
    dtf_features["anova"] = dtf_features["features"].apply(
        lambda x: "anova" if x in anova_selected_features else ""
    )
    dtf_features["num1"] = dtf_features["features"].apply(
        lambda x: 1 if x in anova_selected_features else 0
    )
    dtf_features["lasso"] = dtf_features["features"].apply(
        lambda x: "lasso" if x in lasso_selected_features else ""
    )
    dtf_features["num2"] = dtf_features["features"].apply(
        lambda x: 1 if x in lasso_selected_features else 0
    )
    dtf_features["method"] = dtf_features[["anova", "lasso"]].apply(
    ...
    dtf_feature_importance = pd.DataFrame(
        {"IMPORTANCE": feature_importance * 100, "VARIABLE": feature_names}
    ).sort_values("IMPORTANCE", ascending=False)
    dtf_feature_importance["cumsum"] = dtf_feature_importance["IMPORTANCE"].cumsum(
        axis=0
    )
    ...
    dtf_feature_importance[["IMPORTANCE"]].plot(
        kind="bar", legend=False, ax=ax[0]
    ).grid(axis="y")
    ax[0].set(
        xlabel="",
        xticks=numpy.arange(len(dtf_feature_importance)),
    ...
    dtf_feature_importance[["cumsum"]].plot(
        kind="line",
        linewidth=4,
        legend=False,
        ax=ax[1],
...
def create_model_graphs(
    ...
    ax[1].plot(
        recalls, precisions, color="darkorange", lw=3, label="area = %0.2f" % roc_auc
    )
    ax[1].plot(
        [0, 1],
        [(cm[1, 0] + cm[1, 0]) / len(y_test), (cm[1, 0] + cm[1, 0]) / len(y_test)],
        linestyle="--",
        color="navy",
        lw=3,
    )
    ...
    ax[1].vlines(
        x=recall, ymin=0, ymax=precision, color="red", linestyle="--", alpha=0.7
    )
...
def create_model(
    ...
    print(
        metrics.classification_report(
            y_test, predicted, target_names=[str(i) for i in numpy.unique(y_test)]
        )
    )
    if make_graphs:
        create_model_graphs(
...
def make_prediction(application_ids: list) -> dict:
    ...
    data["label_predicted"] = model.predict_proba(
        data.drop(
            [
                "application1_id",
                "application2_id",
                "application1_endpoint_id",
                "application2_endpoint_id",
                "application1_original_path",
                "application2_original_path",
                "application1_operation",
                "application2_operation",
                "application1_server_override",
                "application2_server_override",
            ],
            axis=1,
        ).values
    )[:, 1]
